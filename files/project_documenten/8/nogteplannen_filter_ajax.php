...
if(!$_POST['action']){
    ...
    if($_GET['filter'] == 'organisatie'){
        $qstring = "SELECT  c.id, c.naam 
                    ...
                    WHERE   a.actief = 1 AND a.status = 'done' AND b.actief = 1 AND c.naam LIKE '%$term%'";}
    ...
    elseif($_GET['filter'] == 'project'){
        ...
        $qstring = "SELECT  b.id, b.titel AS naam 
                    FROM    planning_activiteit a
                            LEFT JOIN project AS b ON (b.id = a.project) 
                    WHERE   a.actief = 1  AND a.status = 'done' AND b.organisatie $organisaties AND b.titel LIKE '%$term%'";
    }
    ...
    elseif($_GET['filter'] == 'competentie'){
        ...
        $qstring = "SELECT b.id, b.competentie AS naam 
                    ...
                    WHERE  a.actief = 1 AND a.status = 'done' AND a.project $projecten AND b.competentie LIKE '%$term%'";
    }
    ...
    elseif($_GET['filter'] == 'medewerker'){
        ...
        $qstring = "SELECT b.id, b.voornaam, b.achternaam 
                    ...
                    WHERE a.actief = 1 AND a.status = 'done' AND a.competentie $competenties AND (b.voornaam LIKE '%$term%' OR b.achternaam LIKE '%$term%') AND (a.in_behandeling_door = b.id OR a.acceptatie_door = b.id)";
    }
    //echo $qstring;
