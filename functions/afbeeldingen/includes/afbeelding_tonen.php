<?php
    ...
    $what = 'a.id, a.path, a.omschrijving, a.volgorde, UNIX_TIMESTAMP(a.geupload_op) AS geupload_op,
    ...
    */
    ...
    $from = 'portal_image_reactie a 
     ...
     $where = 'a.image = '.$foto['id'].' AND b.actief = 1 ORDER BY b.geschreven_op DESC';
...
<div id="foto_div">
    ...
    <?php 
        ...
        $image_span =  '
            <span id="foto_image" style="background: url('.$etc_root.'lib/slir/'.slirImage(600,400).$etc_root.'files/album/'.$foto['album'].'/'.$foto['path'].') no-repeat center center transparent;">
                &nbsp;
            </span>';
        ...
        $image_span =  '
            <span id="foto_image" style="background: url('.$etc_root.'lib/slir/'.slirImage(400,400).$etc_root.'files/album/'.$foto['album'].'/'.$foto['path'].') no-repeat center center transparent;">
                &nbsp;
            </span>';
        ...
        $image_span =  '
            <span id="foto_image" style="background: url('.$etc_root.'lib/slir/'.slirImage(0,400,0).$etc_root.'files/album/'.$foto['album'].'/'.$foto['path'].') no-repeat center center transparent;">
                &nbsp;
            </span>';   
    ...
    <p id="foto_credits">
        <?php
            ...
            Ge&uuml;pload door <span id="profiel"><?php echo $foto['voornaam'].' '.$foto['achternaam']; ?></span> <span id="geupload"><?php echo verstrekenTijd($foto['geupload_op']); ?></span>
        <?php }
            elseif($soort['soort'] == 'nieuws'){?>
            Op <span id="geupload"><?php echo verstrekenTijd($foto['geupload_op']); ?></span> ge&uuml;pload binnen het nieuwsitem <span id="nieuwsitem">'<?php echo $soort['titel']; ?>'</span> 
        <?php }
        ?>
    </p>
    <div id="foto_omschrijving">
    <p><?php echo $foto['omschrijving'] ?></p>
    </div>
    <div id="foto_div_acties">
        ...
        <div id="formulier" style="display: none">
                ...
                <form id="reactie_formulier_formulier" method="post" action="<?php echo $etc_root; ?>functions/afbeeldingen/includes/afbeelding_check.php">
                    <textarea class="textarea" name="reactie" id="reactie" rows="3"></textarea>
            ...
            <div class="reactie <?php echo $class; ?>" onmouseover="this.className='reactie reactie_hover <?php echo $class; ?>'" onmouseout="this.className='reactie <?php echo $class; ?>'">
               <div class="reactie_links">
                ...
                <div class="reactie_rechts">
                        ...
                        <p>
                            ...
                            <span class="delete_reactie">
                                <span class="delete_container">
                                    <img class="delete" onclick="deleteReactie(<?php echo $foto['id'].', \'foto_iframe\','.$reactie['id'] ?>)" src="<?php echo $etc_root; ?>functions/home/css/images/delete.png" alt="X" title="Verwijder reactie">
                                </span>
                            </span>
                            <?php }?>
                    ...
                    echo 'Er zijn geen reactie\'s achtergelaten bij deze foto';
                }
        ?>
        </div>
    </div>
        </div>
        <div id="stel_in_als_profielfoto">
            <h2 id="maak_profielfoto" onmouseover="this.className='maak_profielfoto_hover'" onmouseout="this.className=''">Maak dit uw profielfoto</h2> 
        </div>
    </div>
</div>
...
$('#maak_profielfoto').live("click", function(e){    
        $.ajax({
            type : 'GET',
            url : '<?php echo $etc_root; ?>functions/afbeeldingen/includes/afbeelding_check.php',
            dataType : 'html',
            data: {
                action : 'maakProfielfoto',
                foto_id : '<?php echo $foto['id']; ?>'
            },
            success : function(data){
                $('#foto_div_navigatie').html(data);
