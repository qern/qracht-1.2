...
if($_POST['action'] == 'reactie'){
    ...
    if($_POST['reactie'] != null){        
        ...
        $with_what= $_POST['image_id'].', (SELECT MAX(id) FROM portal_reactie)';
            ...
            $table ="portal_image"; $what = 'update_datum = NOW()'; $where = "id  = ".$_POST['image_id'];
...
elseif($_GET['action'] == 'maakProfielfoto'){
    $table = 'portal_gebruiker'; $what= 'profielfoto = '.$_GET['foto_id'];
    $where = 'id = '.$login_id;
        $update_profielfoto = sqlUpdate($table, $what, $where);
        
    $what = 'id AS profielfoto'; $from = 'portal_gebruiker_recent'; 
       $where = 'gebruiker = '.$login_id.' AND wat = \'profielfoto\'';
        $recent = mysql_fetch_assoc(sqlSelect($what, $from, $where));
        
        if($recent['profielfoto'] != null){
               $table = 'portal_gebruiker_recent'; $what = 'gewijzigd_op = NOW()'; $where = 'gebruiker = '.$login_id.' AND wat = \'profielfoto\'';
                $update_recent = sqlUpdate($table, $what, $where);
        }else{
               $table = 'portal_gebruiker_recent'; $what = 'gebruiker, wat, gewijzigd_op'; 
               $with_what = $login_id.', \'profielfoto\', NOW()';
                $insert_recent = sqlInsert($table, $what, $with_what);
        }
    
    echo '<div class="wijzig_info_success"><h2>U hebt met succes deze foto als profielfoto ingesteld</h2></div>';
}
