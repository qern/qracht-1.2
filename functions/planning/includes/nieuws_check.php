...
require($_SERVER['DOCUMENT_ROOT'].'/portal/check_configuration.php');
...
if($_POST['action'] == 'aanmaken'){
    ...
    }else{$teaser = htmlspecialchars($_POST['teaser']); $_SESSION['teaser'] = $teaser;}
    
    if($_POST['inhoud'] == null){ $error['inhoud'] = 'U dient een inhoud in te voeren';
    }else{$inhoud = $_POST['inhoud']; $_SESSION['inhoud'] = $inhoud;}
    ...
    if($_FILES['file'] != null){
        ...
        require($_SERVER['DOCUMENT_ROOT'].'/portal/lib/afbeelding_uploaden.php');
    ...
    if(!$error){
        ...
        header('location: '.$site_name.'nieuws/wijzigen/nieuws-id='.$nieuws_id);
    }else{
        $_SESSION['error'] = $error;
        header('location: '.$site_name.'nieuws/toevoegen');
    }
...
elseif($_POST['action'] == 'wijzigen'){
    ...
    if($_POST['teaser'] == null){$error['teaser'] = 'U dient een teasertekst in te voeren';
    }else{$teaser = htmlspecialchars($_POST['teaser']);}
    ...
    if($_POST['publicatiedatum'] == null){ $publicatiedatum = strftime('%d-%m-%Y');
    }else{$publicatiedatum = $_POST['publicatiedatum'];}
    
    if($_POST['archiveerdatum'] == null){$archiveerdatum = '00-00-00';
    }else{$archiveerdatum = "STR_TO_DATE('".$_POST['archiveerdatum']."','%d-%m-%Y')";}
