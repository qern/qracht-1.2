...
//haal de verzameling op
    ...
    echo $verzameling_res['verzameling'];
...
if (!empty($_FILES)) {
    ...
    if(is_dir($targetPath)){}else{mkdir($targetPath);} //bestaat de map al ? nee ? aanmaken dan.
    
	$targetFile =  str_replace('//','/',$targetPath) . $_FILES['Filedata']['name'];
    list($album_id, $login_id) = explode('_', $_GET['ids']);
    $filename = $_FILES['Filedata']['name']; //$activiteit_id = $_GET['activiteit_id'];    

    //haal de verzameling op
    $what = 'MAX(verzameling) AS verzameling'; 
    $from = 'portal_image';
    $where = '(UNIX_TIMESTAMP(NOW()) - UNIX_TIMESTAMP(geupload_op)) <= 1800 AND album='.$album_id;
   ...
   //if($verzameling_res['verzameling'] == 0 || $verzameling_res['verzameling'] == null){ $verzameling = '10';}
    ...
    $table='portal_image';
    $what='album, path, volgorde, verzameling, geupload_door, geupload_op';
    $with_what="$album_id, '$filename', '$volgorde' , '$verzameling', '$login_id', NOW()";
        $insert_foto_bestand = sqlInsert($table, $what, $with_what);
