...
//------------------------------------------------
	 ...
	 */
	$what = 'agenda_url'; $from='portal_instellingen'; $where="actief = 1";
    $agenda = mysql_fetch_assoc(sqlSelect($what, $from, $where));
	if($agenda['agenda_url'] != null){
    $calendarfeed = $agenda['agenda_url']; 
...
/*$file = $_SERVER['DOCUMENT_ROOT']."/portal/functions/home/includes/agenda.xml";
$ch = curl_init($calendar_xml_address);
$fp = @fopen($file, 'w');
...
fclose($fp);*/
/*
curl_setopt($ch, CURLOPT_URL, $calendar_xml_address);
curl_setopt($ch, CURLOPT_FAILONERROR, 1);
//curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
curl_setopt($ch, CURLOPT_TIMEOUT, 15);
$calendar_xml_address_curl = curl_exec($ch);
*/

//$calendar_xml_address = file_get_contents($calendar_xml_address_curl);
