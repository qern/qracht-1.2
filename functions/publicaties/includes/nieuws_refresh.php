<?php
  ...
  if($_GET['action'] == 'refresh_file')  {
    ...
    if($_GET['subaction'] == 'delete'){
        ...
        $what_bestanden = 'path'; $from_bestanden = 'portal_nieuws_bestand'; $where_bestanden = 'id ='.$_GET['bestand_id'];
            $te_verwijderen = mysql_fetch_assoc(sqlSelect($what_bestanden, $from_bestanden, $where_bestanden));
        
        //verwijder het bestand fysiek
        unlink($_SERVER['DOCUMENT_ROOT'].$etc_root.'files/bestand/'.$nieuws_id.'/'.$te_verwijderen['path']);
        //echo $_SERVER['DOCUMENT_ROOT'].'<?php echo $etc_root; files/'.$nieuws_id.'/'.$te_verwijderen['path'];
  ...
  elseif($_GET['action'] == 'refresh_foto'){
      ...
      if($_GET['subaction'] == 'delete'){
        ...
        //verwijder het bestand fysiek
        unlink($_SERVER['DOCUMENT_ROOT'].$etc_root.'files/album/'.$album_id.'/'.$te_verwijderen['path']);
