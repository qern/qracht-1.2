...
$path = $_SERVER['DOCUMENT_ROOT'].$etc_root.'files/bestand/'.$_GET['nieuws'].'/';
if(is_dir($path)){}else{mkdir($path);} //bestaat de map al ? nee ? aanmaken dan.
$uploader = new qqFileUploader($allowedExtensions, $sizeLimit);
$result = $uploader->handleUpload($path);

   $what = 'MAX(verzameling) AS verzameling'; 
   $from = 'portal_nieuws_bestand';
   $where = '(UNIX_TIMESTAMP(NOW()) - UNIX_TIMESTAMP(toegevoegd_op)) <= 2400 AND nieuws='.$_GET['nieuws'];
   ...
   if($verzameling_res['verzameling'] == null){ 
       $what = 'MAX(verzameling) + 1 AS verzameling'; $from = 'portal_nieuws_bestand'; $where = '1';
    ...
    $table='portal_nieuws_bestand';
    $what='path, nieuws, verzameling, toegevoegd_door, toegevoegd_op';
    $with_what="'".$_GET['qqfile']."', '".$_GET['nieuws']."', $verzameling, '$login_id', NOW()";
