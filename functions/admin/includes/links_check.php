<?php 
    ...
    elseif($_POST['action'] == 'createlink'){
        ...
        if(!$error){ //zijn er geen fouten, wijzig de database dan
           ...
           $with_what = '\''.mysql_real_escape_string($url).'\', \''.mysql_real_escape_string($beschrijving).'\', \''.mysql_real_escape_string($_POST['categorie']).'\', 999, NOW()';
                $nieuwe_link = sqlInsert($table, $what, $with_what);
           
           $table = 'portal_gebruiker_link';
           $what = 'gebruiker, link';
           $with_what = '999, (SELECT MAX(id) FROM portal_link WHERE actief = 1)';
                $nieuwe_gebruiker_link_koppeling = sqlInsert($table, $what, $with_what);
           
           //laat de gebruiker weten dat het goed is gegaan
           $message = '<div class="wijzig_info_success">De link is toegevoegd</div>';
    ...
    elseif($_POST['action'] == 'deletelink'){
        $table = 'portal_gebruiker_link'; $where = 'link = '.$_POST['link_id'].' AND gebruiker = 999';
        ...
        echo '<div class="wijzig_info_success">Betreffende kennis verwijderd</div>';        
    }
    ...
    elseif($_POST['action'] == 'sort_link'){
        ...
        foreach($sort_afbeelding['link'] as $positie => $id){
            ...
            $where = "link = $id AND gebruiker = 999";
